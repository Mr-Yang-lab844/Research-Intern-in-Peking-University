...
# -*- coding: utf-8 -*-
"""
选择题层24消融实验：固定特征数=9，改变检索数量(1-10)和随机种子(42,123,2024)
...
TOTAL_SAMPLES = 100
DEV_SIZE = 50
FEATURE_COUNT = 9          # 根据特征数量消融得到的最佳值
TOP_K_LIST = list(range(1, 11))   # 1~10
SEED_LIST = [42, 123, 2024]

# 缓存目录
CACHE_DIR = "./cache/choice_layer24"
...
for top_k in TOP_K_LIST:
    for seed in SEED_LIST:
        ...
        if os.path.exists(cache_file):
            ...
            test_true_error = data['test_true_error']
            top_indices = data['top_indices']
            top_weights = data['top_weights']
            auc = data['auc']
        ...
        # 处理可能的情况：如果开发集中没有错误样本或没有正确样本，则跳过
        if correct_mask.sum() == 0 or error_mask.sum() == 0:
            print(f"警告: 开发集中正负样本不足 (correct={correct_mask.sum()}, error={error_mask.sum()}), 跳过")
            continue
        ...
        test_true_error = np.array(test_labels)   # 已经是1表示错误
