...
# -*- coding: utf-8 -*-
"""
UltraMedical 自由文本有 RAG 检索数量消融（层10）
- 固定特征数=15（来自全层扫描默认值）
- 检索 Top‑K=1..10，固定划分
"""
...
DEVICE = "cuda"
LAYER = 10
TOP_FEATURES = 15          # 固定特征数
TOP_K_LIST = list(range(1, 11))

CACHE_DIR = "./cache_ultra_free_rag_topk"
...
def get_feature(report_text, top_k):
    retrieved = vector_store.similarity_search(report_text, k=top_k)
...
for top_k in TOP_K_LIST:
    ...
    np.savez(cache_file, auc=auc, dev_labels=dev_labels, test_true_error=test_true_error)
    results[top_k] = auc
