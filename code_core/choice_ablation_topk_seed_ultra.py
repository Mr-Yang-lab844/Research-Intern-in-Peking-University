...
TRAIN_FILE = "./data/train.jsonl"                    # 同源训练集
...
# 构建或加载 FAISS 索引（教科书+参考范围+同源训练集）
def get_index():
    index_path = "./faiss_choice_ultra_rag"
    if os.path.exists(index_path):
        ...
        print("构建 RAG 索引（教科书+参考范围+同源训练集）...")
        docs = []
...
# 加载所有样本（CMExam test.jsonl 有 6606 条，足够采样）
...
print("\n=== 消融实验汇总 (Llama-3.1-8B-UltraMedical, 层27, 特征数=13, 样本量=200) ===")
